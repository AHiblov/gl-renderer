...
void Base3DRenderer::renderTerrainDirectional(int index)
{
	...
	glEnableVertexAttribArray(COMPONENT_VERTEX);
	glBindBuffer(GL_ARRAY_BUFFER, terrainData.vertexBufferId);
	glVertexAttribPointer(COMPONENT_VERTEX, 3, GL_FLOAT, GL_FALSE, 0, (void*) nullptr);

	glEnableVertexAttribArray(COMPONENT_UV);
	glBindBuffer(GL_ARRAY_BUFFER, terrainData.uvBufferId);
	glVertexAttribPointer(COMPONENT_UV, 2, GL_FLOAT, GL_FALSE, 0, (void*) nullptr);

	glEnableVertexAttribArray(COMPONENT_NORMAL);
	glBindBuffer(GL_ARRAY_BUFFER, terrainData.normalBufferId);
	glVertexAttribPointer(COMPONENT_NORMAL, 3, GL_FLOAT, GL_FALSE, 0, (void*) nullptr);
	...
	glDisableVertexAttribArray(COMPONENT_NORMAL);
	glDisableVertexAttribArray(COMPONENT_UV);
	glDisableVertexAttribArray(COMPONENT_VERTEX);
}
...
void Base3DRenderer::renderObjectsDirectionalNormalmap(const ObjectQuadSubdivision::ObjectNode &objectInstances)
{
	...
	glEnableVertexAttribArray(COMPONENT_TANGENT);
	glBindBuffer(GL_ARRAY_BUFFER, objectData.tangentBufferId);
	glVertexAttribPointer(COMPONENT_TANGENT, 3, GL_FLOAT, GL_FALSE, 0, (void*) nullptr);

	glEnableVertexAttribArray(COMPONENT_BITANGENT);
	glBindBuffer(GL_ARRAY_BUFFER, objectData.bitangentBufferId);
	glVertexAttribPointer(COMPONENT_BITANGENT, 3, GL_FLOAT, GL_FALSE, 0, (void*) nullptr);
	...
	glBindTexture(GL_TEXTURE_2D, objectData.normalTextureId);
	...
	glDisableVertexAttribArray(COMPONENT_BITANGENT);
	glDisableVertexAttribArray(COMPONENT_TANGENT);
	glDisableVertexAttribArray(COMPONENT_NORMAL);
	glDisableVertexAttribArray(COMPONENT_UV);
	glDisableVertexAttribArray(COMPONENT_VERTEX);
}
...
void Base3DRenderer::renderObjectsDirectionalInstancing(const ParticleQuadSubdivision::ParticleNode &instanceGroup)
{
	...
	glEnableVertexAttribArray(COMPONENT_INSTANCE_OFFSET);
	glBindBuffer(GL_ARRAY_BUFFER, particleData.arrangementBufferId);
	glVertexAttribPointer(COMPONENT_INSTANCE_OFFSET, 3, GL_FLOAT, GL_FALSE, 0, (void*) nullptr);

	glEnableVertexAttribArray(COMPONENT_INSTANCE_ROTATION);
	glBindBuffer(GL_ARRAY_BUFFER, particleData.rotationBufferId);
	glVertexAttribPointer(COMPONENT_INSTANCE_ROTATION, 1, GL_FLOAT, GL_FALSE, 0, (void*) nullptr);

	glVertexAttribDivisor(COMPONENT_VERTEX, 0); //Reuse the same data
	glVertexAttribDivisor(COMPONENT_UV, 0);
	glVertexAttribDivisor(COMPONENT_NORMAL, 0);
	glVertexAttribDivisor(COMPONENT_INSTANCE_OFFSET, 1); //One per instance
	glVertexAttribDivisor(COMPONENT_INSTANCE_ROTATION, 1);
    ...
    glDisableVertexAttribArray(COMPONENT_INSTANCE_ROTATION);
	glDisableVertexAttribArray(COMPONENT_INSTANCE_OFFSET);
	glDisableVertexAttribArray(COMPONENT_NORMAL);
	glDisableVertexAttribArray(COMPONENT_UV);
	glDisableVertexAttribArray(COMPONENT_VERTEX);
}
