...
void main()
{
	...
	vec4 sumUpColour;
	sumUpColour = texture(renderedTexture, passUv + vec2(-xStep, -yStep)) * embossKernel[0][0] + texture(renderedTexture, passUv + vec2(0, -yStep)) * embossKernel[0][1] + texture(renderedTexture, passUv + vec2(xStep, -yStep)) * embossKernel[0][2];
	...
	sumUpColour += texture(renderedTexture, passUv + vec2(-xStep, yStep)) * embossKernel[2][0] + texture(renderedTexture, passUv + vec2(0, yStep)) * embossKernel[2][1] + texture(renderedTexture, passUv + vec2(xStep, yStep)) * embossKernel[2][2];
	sumUpColour /= 8.0;
	sumUpColour += 0.5;
